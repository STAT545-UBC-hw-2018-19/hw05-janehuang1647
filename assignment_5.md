...
we have access the plpyly graph with `ggplotly(new_graph)` locally, however, this is not compatible with the output github\_document, therefore, the code is commented out. The generated html file is in the following link, however, it might not be viewable in the github. [link to the plotly garph](https://github.com/STAT545-UBC-students/hw05-janehuang1647/blob/master/new_graph.html)
